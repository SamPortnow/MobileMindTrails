...
jsPsych.plugins["button-response"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // add in the wait
    var start_pos = trial.stimulus.indexOf('>') + 1;
    var end_pos = trial.stimulus.indexOf('<',start_pos);
    var sentence = trial.stimulus.substring(start_pos,end_pos);
    ...
    setTimeout(function() {
        $('.jspsych-button-response-stimulus').off('click').attr('disabled', 'disabled');
    }, wait);   

    // function to handle responses by the subject
    function after_response(choice) {

      if (Date.now() - trial_start > wait)
      {
